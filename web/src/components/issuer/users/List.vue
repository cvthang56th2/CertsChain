  ...
  <div>
    ...
    <div class="px-8 py-4 mx-auto bg-white rounded-lg shadow-md dark:bg-gray-800 mb-4">
      <div class="flex border-b-2 py-2 font-bold">
        <div class="w-1/5 whitespace-pre-wrap px-2">Name</div>
        <div class="w-1/5 whitespace-pre-wrap px-2">Email</div>
        <div class="w-1/5 whitespace-pre-wrap px-2">Created At</div>
        <div class="w-1/5 whitespace-pre-wrap px-2">Status</div>
        <div class="w-1/5 whitespace-pre-wrap px-2">Actions</div>
      </div>
      <div v-for="(userObj, uIndex) in computedUsers" :key="`user-${uIndex}`" class="flex border-b-2 last:border-b-0 py-2">
        <div class="w-1/5 whitespace-pre-wrap px-2">{{ [userObj.firstName, userObj.lastName].filter(Boolean).join(' ') }}</div>
        <div class="w-1/5 whitespace-pre-wrap px-2">{{ userObj.email }}</div>
        <div class="w-1/5 whitespace-pre-wrap px-2">{{ formatDate(userObj.createdAt) }}</div>
        <div class="w-1/5 whitespace-pre-wrap px-2">
